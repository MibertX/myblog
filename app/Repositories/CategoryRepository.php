...
{
	/**
	 * Path to file of ru translation.
	...
	/**
	 * Path to file of en translation.
	 * 
	 * @var string
	 */
	...
	/**
	 * Injection the dependencies in constructor
	 *
	...
	{
		return $this->prepareCategoriesQuery()->orderBy('categories.name', 'asc')->get();
	}

	/**
	 * Get all gategories for adminzone
	 *
	 * @param string $ordered_collumn
	...
	/**
	 * Get all categories that belongs to post with selected @param $id
	 * 
	 * @param $id
	...
	{
		return $this->model->select(DB::raw('COUNT(category_id) as counter'))
			->where('seen', '=', false)->first();
	}

	public function toogleCategoryActive($data)
	{
		$active_value = $data->active == 'true';
		$this->model->where('category_id', '=', $data->category_id)
			->update(['active' => $active_value]);
	}
	
	public function toogleCategorySeen($data)
	{
	...
	/**
	 * Prepare query for categories. It also includes counted posts for each category
	 * 
	 * @return mixed
	 */
	protected function prepareCategoriesQuery()
	{
		$query = $this->model
			...
			->select('categories.category_id', 'categories.name', 'categories.seen', 
				'categories.active', DB::raw('count(post_category.post_id) as posts'))
			->groupBy('categories.category_id', 'categories.name', 'categories.seen', 'categories.active');
